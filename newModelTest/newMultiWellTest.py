...
new_model = load_fmu(compile_fmu("newMultiWellTest", compiler_options = {'extra_lib_dirs':extra_lib_dirs}, compiler_log_level = 'e'))
opts = new_model.simulate_options()
opts['CVode_options']['rtol'] = 1e-6

new_model_result = new_model.simulate(final_time = 10000, options = opts)
...
for i in range(1,int(new_model_result['n_wells'])+1):
	...
	p_bh.append(new_model_result['well['+`i`+'].tub.p_bh'][-1])
